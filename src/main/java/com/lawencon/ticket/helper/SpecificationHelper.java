...
import com.lawencon.ticket.model.request.SortByDirection;
import com.lawencon.ticket.validation.annotation.SortProperty;
...
public class SpecificationHelper {
    ...
    private static <T, Y> Join<T, Y> getJoin(Root<T> root, String column) {
        return (Join<T, Y>) root.getJoins().stream()
                .filter(r -> r.getAttribute().getName().equals(column)
                        && r.getJoinType().equals(JoinType.LEFT))
    ...
    public static <T> Specification<T> inquiryFilterOld(List<String> columns, String inquiry) {
        return ((root, query, cb) -> {
            ...
            for (String column : columns) {
                ...
                predicates.add(cb.and(cb.like(cb.lower(path.as(String.class)),
                        "%|" + inquiry.toLowerCase() + "%", '|')));
            }
    ...
    public static <T> Specification<T> inquiryFilter(List<String> columns, String inquiry) {
        return ((root, query, cb) -> {
            ...
            Expression<String> newColumn = cb.function("concat_ws", String.class,
                    concatColumns.toArray(new Expression[0]));
            return cb.like(cb.lower(newColumn), "%|" + inquiry.toLowerCase() + "%", '|');
        });
    }

    public static <T, Y> Specification<T> subqueryIn(String targetColumn, String subqueryColumn,
            Class<Y> subqueryClass, Specification<Y> subquerySpec) {
        ...
        return ((root, query, cb) -> {
            ...
            subquery.select(SpecificationHelper.getRootExpression(subqueryColumn, subqueryRoot))
    ...
    public static <T> Specification<T> parameterFilterIfConditionTrue(Boolean condition,
    ...
    public static <T> Specification<T> parameterFilterIfNotNull(String column, Object value,
        ...
        return ((root, query, cb) -> {
            ...
            Expression<Boolean> expression = cb.<Boolean>selectCase()
                    .when(path.isNotNull(), createPredicate(cb, path, value, operator))
                    .otherwise(true);
    ...
    public static <T> Specification<T> ifNotNullEqualTo(String column, Object value) {
        return ((root, query, cb) -> {
            ...
            Expression<Boolean> expression = cb.<Boolean>selectCase()
                    .when(path.isNotNull(), cb.equal(path, value)).otherwise(true);
            return cb.and(expression, cb.literal(true));
        });
    }

    public static <T> Specification<T> ifNotNullEqualToElse(String column1, String column2,
            Object value) {
        return ((root, query, cb) -> {
            Path<T> path = getRootPath(column1, root);

            Expression<Boolean> expression =
                    cb.<Boolean>selectCase().when(path.isNotNull(), cb.equal(path, value))
    ...
    public static <T> Specification<T> likeAny(String column, Object value) {
        return ((root, query, cb) -> {
            ...
            return cb.like(
                    cb.lower(cb.function("array_to_string", String.class, pathArray,
                            cb.<String>literal("|"))),
    ...
    public static <T> Specification<T> arrayStringIsContainedBy(String column,
            List<String> values) {
        return ((root, query, cb) -> {
            Expression<T> pathArray = getRootExpression(column, root);
    ...
    public static <T> Specification<T> ifNullIsNullOrElse(String column, Object value,
            SpecificationOperation operator) {
        if (value == null) {
    ...
    @SuppressWarnings({"rawtypes", "unchecked"})
    private static Predicate createPredicate(CriteriaBuilder cb, Expression column,
            Expression column2, SpecificationOperation operator) {
        switch (operator) {
            case EQUAL:
            ...
            case MATCH:
                return cb.like(column,
                        cb.function("concat_ws", String.class, cb.<String>literal(""),
                                cb.<String>literal("%"), column2, cb.<String>literal("%")));
            case MATCH_END:
                return cb.like(cb.lower(column),
                        cb.function("concat_ws", String.class, cb.<String>literal(""),
                                cb.<String>literal("%"), cb.lower(column2),
                                cb.<String>literal("%")));
            case NOT_EQUAL:
    ...
    public static Sort createSort(List<SortBy> sortBy) {
        ...
        sortBy.stream().forEach(s -> orders.add(
                new Order(Direction.fromString(s.getDirection().name()), s.getPropertyName())));
        return Sort.by(orders);
    }
    ...
    public static Sort createSort(Class<?> clazz, List<SortBy> sortBy) {
        ...
        List<Order> orders = new ArrayList<>();
        sortBy.stream()
                .forEach(s -> list.stream().filter(f -> f.getName().equals(s.getPropertyName()))
    ...
    public static <T> Comparator<T> createModelSort(Class<T> clazz, Sort sort) {
        return (val1, val2) -> {
            ...
            List<Field> list = getAllBaseField(clazz);
            sort.forEach(s -> list.stream().filter(f -> f.getName().equals(s.getProperty()))
