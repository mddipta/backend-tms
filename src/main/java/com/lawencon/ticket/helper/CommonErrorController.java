...
import com.lawencon.ticket.error.ConstraintValidationException;
import com.lawencon.ticket.model.response.WebResponse;
import com.lawencon.ticket.validation.ValidationErrorMapper;
...
public class CommonErrorController implements MessageSourceAware {
    ...
    @ExceptionHandler(
            value = {WebExchangeBindException.class, MethodArgumentNotValidException.class})
    public ResponseEntity<WebResponse<Object>> webExchangeBindException(
            WebExchangeBindException e) {
        log.warn(WebExchangeBindException.class.getName(), e);

        var httpStatus = HttpStatus.valueOf(e.getStatusCode().value());
        var response = WebResponse.builder().code(httpStatus.value()).status(httpStatus.name())
                .errors(ValidationErrorMapper.getErrorsFrom(e.getBindingResult(), messageSource))
                .build();
    ...
    public ResponseEntity<WebResponse<Object>> missingServletRequestParameterException(
            MissingServletRequestParameterException e) {
        log.warn(MissingServletRequestParameterException.class.getName(), e);
        var errors = Map.ofEntries(
                Map.entry(e.getParameterName(), Collections.singletonList("is required")));
        return ResponseEntity.badRequest().body(ResponseHelper.badRequest(errors));
    }
